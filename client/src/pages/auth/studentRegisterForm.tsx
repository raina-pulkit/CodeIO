...
import { studentRegistrationSchema } from "@/../utils/validationSchema";
...
const StudentRegisterForm = () => {
  ...
  const errorShower = async (title: string, msg: string, variant?: "default" | null) => {
    toast({
  ...
  const handleSubmitData = async (
    body: z.infer<typeof studentRegistrationSchema>,
  ...
  return !loading ? (
    <Card className="bg-muted-foreground">
      ...
      <CardContent className="space-y-2">
        <Form {...form}>
          <form
            onSubmit={handleSubmit(handleSubmitData, (e) =>
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  {/* Render allows for an error message to be shown everytime there's an error on this particular input field */}
                  <FormLabel className="text-white">Admission Date</FormLabel>
                  {/* Form Control allows to share the context to display errors */}
                  <FormControl>
                    <Input
            ...
            >
              {buttonLoading ? (
                <div className="h-full w-full flex justify-center items-center py-2">
                  <span className="loading loading-spinner loading-md bg-white text-white"></span>
                </div>
              ) : (
                "Register"
              )}
